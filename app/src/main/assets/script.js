...
function loadActivityFromURL() {
    ...
    const activityId = params.get("activity") || window.__nativeActivityId;
...
function syncTasksToAndroidWidget() {

    try {
        if (window.AndroidAgenda && typeof window.AndroidAgenda.syncTasks === "function") {
            window.AndroidAgenda.syncTasks(JSON.stringify(tasks));
        }
    } catch (error) {
